...
const REPHRASE_USER_PROMPT =
    `Analyze the following text and list 0-3 sentences that should be rephrased for brevity.
...
export const acceptRevision = (quill: Quill, revision: Revision) => {
    ...
    const plainContent = content.replace(/<\/p><p>/g, "\n").replace(
        /<[^>]*>/g,
        "",
    );
...
export const publishRevisions = (
    ...
    for (const suggestion of revisions) {
        ...
        quill.formatText(index, quote.length, "background", FORMAT_COLORS[keyword]);
    }
    dispatch({
        type: "set",
        key: keyword,
...
export const update = async (
    ...
    if (state.enabled.rephrase) {
        const rephrases = await getRephrases(
            content,
            state.apiKey,
        );
    ...
    if (state.enabled.factCheck) {
        const factSuggestions = await getFacts(
            content,
            state.apiKey,
        );
